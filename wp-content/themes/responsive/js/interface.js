...
(function( window, $, undefined ) {
	...
	$.iUtil = {
		...
		getPointer: function (event) {
			var x = event.clientX || event.pageX || (event.clientX + (document.documentElement.scrollLeft || document.body.scrollLeft)) || 0;
			var y = event.clientY || event.pageY || (event.clientY + (document.documentElement.scrollTop || document.body.scrollTop)) || 0;
			return {
				x: x,
					y: y
			}
		...
		traverseDOM: function (nodeEl, func) {
			...
			while (nodeEl) {
				$.iUtil.traverseDOM(nodeEl, func);
				nodeEl = nodeEl.nextSibling
			}
		},
		purgeEvents: function (nodeEl) {
			$.iUtil.traverseDOM(nodeEl, function (el) {
				for (var attr in el) {
	...
	$.fx.destroyWrapper = function (e, old) {
		var c = e.firstChild;
		var cs = c.style;
	...
	$.fx.buildWrapper = function (e) {
		if (!$.fxCheckTag(e)) return false;
		var t = $(e);
		var es = e.style;
		var restoreStyle = false;
		var oldStyle = {};
		...
		oldStyle.sizes = $.iUtil.getSize(e);
		oldStyle.margins = $.iUtil.getMargins(e);
	...
	$.fx.parseColor = function (color, notColor) {
		if ($.fx.namedColors[color]) return {
			r: $.fx.namedColors[color][0],
				g: $.fx.namedColors[color][1],
					b: $.fx.namedColors[color][2]
		};
	...
	$.fx.cssSides = ['Top', 'Right', 'Bottom', 'Left'];
	$.fx.cssSidesEnd = {
	...
	$.extend({
		...
		values;
	var y = elem.style;
	var oldOverflow = $.css(elem, "overflow");
	var oldDisplay = $.css(elem, "display");
	var props = {};
	z.startTime = (new Date()).getTime();
	options.easing = options.easing && $.easing[options.easing] ? options.easing : 'linear';
	z.getValues = function (tp, vp) {
		if ($.fx.cssProps[tp]) {
			if (vp == 'show' || vp == 'hide' || vp == 'toggle') {
				if (!elem.orig) elem.orig = {};
				var r = parseFloat($.curCSS(elem, tp));
				elem.orig[tp] = r && r > -10000 ? r : (parseFloat($.css(elem, tp)) || 0);
				...
				else $.attr(y, "opacity", props[tp][0])
			} else {
				props[tp] = [parseFloat($.curCSS(elem, tp)), parseFloat(vp) || 0]
			}
		} else if ($.fx.colorCssProps[tp]) props[tp] = [$.fx.parseColor($.curCSS(elem, tp)), $.fx.parseColor(vp)];
			...
			var m = vp.replace(/\s+/g, ' ').replace(/rgb\s*\(\s*/g, 'rgb(').replace(/\s*,\s*/g, ',').replace(/\s*\)/g, ')').match(/([^\s]+)/g);
					switch (tp) {
						...
						case 'borderColor':
							...
							m[2] = m[2] || m[0];
							m[1] = m[1] || m[0];
							for (var i = 0; i < $.fx.cssSides.length; i++) {
								var nmp = $.fx.cssSidesEnd[tp][0] + $.fx.cssSides[i] + $.fx.cssSidesEnd[tp][1];
								props[nmp] = tp == 'borderColor' ? [$.fx.parseColor($.curCSS(elem, nmp)), $.fx.parseColor(m[i])] : [parseFloat($.curCSS(elem, nmp)), parseFloat(m[i])]
							}
							break;
						case 'border':
							for (var i = 0; i < m.length; i++) {
								...
								if (sideEnd) {
									for (var j = 0; j < $.fx.cssSides.length; j++) {
										nmp = 'border' + $.fx.cssSides[j] + sideEnd;
										props[nmp] = sideEnd == 'Color' ? [$.fx.parseColor($.curCSS(elem, nmp)), $.fx.parseColor(m[i])] : [parseFloat($.curCSS(elem, nmp)), floatVal]
									}
								} else {
									y['borderStyle'] = m[i]
								}
	...
	for (p in prop) {
		if (p == 'style') {
			var newStyles = $.parseStyle(prop[p]);
			for (np in newStyles) {
						...
						for (var j = 0; j < cssRules.length; j++) {
							if (cssRules[j].selectorText == '.' + prop[p]) {
								var rule = new RegExp('\.' + prop[p] + ' {');
									...
									var newStyles = $.parseStyle(styles.replace(rule, '').replace(/}/g, ''));
								for (np in newStyles) {
									this.getValues(np, newStyles[np])
								}
							}
	...
	z.step = function () {
		...
		if (t > options.duration + z.startTime) {
			...
			z.timer = null;
			for (p in props) {
				if (p == "opacity") $.attr(y, "opacity", props[p][1]);
						...
						if (p == "opacity") $.attr(y, p, elem.orig[p]);
						else y[p] = "";
						y.display = options.hide ? 'none' : (oldDisplay != 'none' ? oldDisplay : 'block');
						y.overflow = oldOverflow;
						elem.animationHandler = null;
						if ($.isFunction(options.complete)) options.complete.apply(elem)
						} else {
							var n = t - this.startTime;
							var pr = n / options.duration;
							for (p in props) {
								if (typeof props[p][1] == 'object') {
									y[p] = 'rgb(' + parseInt($.easing[options.easing](pr, n, props[p][0].r, (props[p][1].r - props[p][0].r), options.duration)) + ',' + parseInt($.easing[options.easing](pr, n, props[p][0].g, (props[p][1].g - props[p][0].g), options.duration)) + ',' + parseInt($.easing[options.easing](pr, n, props[p][0].b, (props[p][1].b - props[p][0].b), options.duration)) + ')'
										} else {
											var pValue = $.easing[options.easing](pr, n, props[p][0], (props[p][1] - props[p][0]), options.duration);
											if (p == "opacity") $.attr(y, "opacity", pValue);
	...
	$.parseStyle = function (styles) {
		...
		if (typeof styles == 'string') {
			...
			for (var i = 0; i < styles.length; i++) {
				rule = styles[i].split(':');
				if (rule.length == 2) {
					newStyles[$.trim(rule[0].replace(/\-(\w)/g, function (m, c) {
		...
		build: function (options) {
			...
			maxWidth: options.maxWidth
				...
				el.fisheyeCfg.items.bind('mouseover', function () {
					...
					$(el.fisheyeCfg.itemsText, this).get(0).style.display = 'none'
				});
				$(document).bind('mousemove', function (e) {
					...
					el.fisheyeCfg.items.each(function (nr) {
						distance = Math.sqrt(Math.pow(posy - nr * el.fisheyeCfg.itemWidth -nr * el.fisheyeCfg.itemPadding * 2, 2) + posx);
						distance -= el.fisheyeCfg.itemWidth / 2;
						distance = distance < 0 ? 0 : distance;
						distance = distance > el.fisheyeCfg.proximity ? el.fisheyeCfg.proximity : distance;
						distance = el.fisheyeCfg.proximity - distance;
						extraWidth = el.fisheyeCfg.maxWidth * distance / el.fisheyeCfg.proximity;
						var diameter = el.fisheyeCfg.itemWidth + extraWidth;	
						this.style.width = diameter + 'px';
						this.style.height = diameter + 'px';
						this.style.top = el.fisheyeCfg.itemWidth * nr + toAdd + 'px';
						this.parentNode.style.borderRadius = diameter/2 + 5 + 'px';
						toAdd += extraWidth
					});
					$.iFisheye.positionContainer(el, toAdd)
				})
			})
		},
