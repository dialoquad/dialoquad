...
(function( window, $, undefined ) {
	$(window).load(function() {
		...
		function distanceBubbleTop() {
			var progress = $(document).scrollTop() / (documentHeight - viewportHeight),
					...
					distance = 0;
			//alert(documentHeight + " " + viewportHeight + "\n" + scrollbarHeight + "\n" + progress);
		...
		function timeToRead(array) {
			...
			array.each(function() {
				total += Math.round(60*$(this).text().length / 200); 
				//alert($(this).text().length);
				// 200 = number of words per minute
			});	
	...
	function Time() {
	 ...
	      ' minutes reading time': ' Minuten Lesezeit'
			...
			'zh': {
				'Thank you.': 'Thank you.',
				'less than 1 minute left': '再念幾秒',
				'1 minute left': '再念 1 分鐘',
				' minutes left':' 分鐘剩餘',
				'1 minute reading time': '1 分鐘閱讀時間',
				' minutes reading time': ' 分鐘閱讀時間'
			}
		}
